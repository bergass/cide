...
FUNCTION get_status(p_order_id orders.order_id%TYPE)
    RETURN orders.status%type;
    
FUNCTION get_products_donat_order_id(p_order_id orders.order_id%TYPE)
...
CREATE OR REPLACE PACKAGE BODY orders_mgmt AS

FUNCTION get_status(p_order_id orders.order_id%TYPE) RETURN orders.status%type IS
    vStatus orders.status%type;
    BEGIN
    SELECT STATUS INTO vStatus FROM orders WHERE ORDER_ID = p_order_id;
    RETURN vStatus;
    END;
    
FUNCTION get_products_donat_order_id(p_order_id orders.order_id%TYPE) RETURN products_table IS
    BEGIN
    END;
    
    BEGIN
    --Part d’inicialització
END orders_mgmt;
